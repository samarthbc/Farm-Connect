...
function Farmer_Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(`${server_url}/api/farmerauth/farmeredit`, {
      method: 'POST',
      headers: {
    ...
    const response1 = await fetch(`${server_url}/api/farmerauth/pfp`, {
      method: "POST",
      headers: {
        'auth-token': localStorage.getItem('token')
      },
  ...
  const getcurrent = async () => {
    ...
    let currentfarmer = await response.json();
    setCredentials(currentfarmer);
  }
